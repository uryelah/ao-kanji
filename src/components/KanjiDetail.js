...
class KanjiDetail extends Component {
  ...
  details() {
    return (
      <Fragment>
        ...
        <h3>{this.props.state.subscription.kanji.kunyomi.hiragana} - {this.props.state.subscription.kanji.onyomi.katakana}</h3>
        <h2>{this.props.state.subscription.kanji.meaning.english}</h2>
        <hr/>
        <article>
          Strokes
          <div>
            {this.props.state.subscription.kanji.strokes.images.map(image => {
              return <img src={image} width='60'></img>
            })}
          </div>
        </article>
        <hr/>
        <strong>Radical: {this.props.state.subscription.radical.character}</strong>
        <hr/>
        <article>
          <h3>Examples</h3>
          <ul>
          {this.props.state.subscription.examples.map(example => {
            return(
            <li>
              <strong>{example.japanese}</strong> - <em>{example.meaning.english}</em>
              <div>
                <audio controls>
  ...
  render() {
    if (this.props.state.subscription) {
      const { kanji, examples, radical, references } = this.props.state.subscription;
